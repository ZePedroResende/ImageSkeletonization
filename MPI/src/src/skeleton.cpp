...
void send_matrix_middle(int rank, int size){
    free(aux);
    aux = (int *) malloc(width * height * sizeof(int));
    memset(aux, 0, width * height * sizeof(int));
...
void send_matrix_last(int size){
    free(aux);
    aux = (int *) malloc(width * height * sizeof(int)); 
...
void middle_process(int rank, MPI_Status status){
    int hi = rank * h;
    height_process = h + 2;
    ...
    memcpy( matrix, &ret[(hi-1)*width], width * height_process * sizeof(int));
    memcpy( aux, &ret[(hi-1)*width], width * height_process * sizeof(int));
...
void last_process(int rank, MPI_Status status){
...
void process_zero(int rank, MPI_Status status){
    height_process = h+1;
    free(aux);
...
int main(int argc, char *argv[]){
  ...
  if (rank == 0) {
    ...
    last_process(rank, status);

  }else{
    
    middle_process(rank, status);
  ...
  if ( rank==0 ) {
    ...
    send_matrix_last(size);

  }else{
    
    send_matrix_middle(rank, size);
